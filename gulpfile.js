let project_folder = "dist"; //require("path").basename(__dirname); меняет название папки dist на название проэкта
...
let { src, dest } = require("gulp"),
  ...
  fonter = require("gulp-fonter"); //конвектирует шрифты с otf в ttf
...
function js() {
  return src(path.src.js)
    ...
    .pipe(dest(path.build.js)) //путь куда выгрузиться js
    .pipe(browsersync.stream());
}
...
function images() {
  return src(path.src.img)
    ...
    .pipe(dest(path.build.img)) //путь куда выгрузяться картинки
    .pipe(browsersync.stream());
}
